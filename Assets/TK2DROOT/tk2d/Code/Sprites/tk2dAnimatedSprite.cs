...
public class tk2dAnimatedSprite : tk2dSprite
{
	...
	/// <summary>
	/// Randomly select from clips on the animation
	/// </summary>
	...
	new void Start()
	{
		...
		animationEventDelegate += PlayFrameAudio;
		
		if (playAutomatically) {
			if(randomClip) {
				Play(Random.Range(0,anim.clips.Length));
			} else {
				Play(clipId);
			}
		}
	...
	private static void PlayFrameAudio(tk2dAnimatedSprite sprite, tk2dSpriteAnimationClip clip, tk2dSpriteAnimationFrame frame, int frameNum)
	{
		//spotco -- log here
	}
}
