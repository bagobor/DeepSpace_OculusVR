...
public class tk2dSpriteAnimationEditorPopup : EditorWindow 
{
	...
	private GUIStyle SC_ListBoxItemSelected = null;
	...
	void DrawList()
	{
		...
		foreach (tk2dSpriteAnimationClip clip in filteredClips)
		{
			...
			var skin = tk2dEditorSkin.SC_ListBoxItem;
			if(selected)
			{
				if(SC_ListBoxItemSelected == null)
				{
					SC_ListBoxItemSelected = new GUIStyle(tk2dEditorSkin.SC_ListBoxItem);
					SC_ListBoxItemSelected.fontStyle = FontStyle.Bold;
				}
				skin = SC_ListBoxItemSelected;
			}
			
			bool newSelected = GUILayout.Toggle(selected, clip.name, skin, GUILayout.ExpandWidth(true));
